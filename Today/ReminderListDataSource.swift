...
class ReminderListDataSource: NSObject {
    ...
    typealias ReminderChangedAction = () -> Void
...
extension ReminderListDataSource: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let dataText = currentReminder.dueDateTimeText(for: filter)
        cell.configure(title: currentReminder.title,
                       dateText: dataText,
...
extension Reminder {
    ...
    static let todayDateFormatter: DateFormatter = {
        let format = NSLocalizedString("'Today at '%@",
                                       comment: "format string for dates occurring today")
        let dateFormatter = DateFormatter()
    ...
    func dueDateTimeText(for filter: ReminderListDataSource.Filter) -> String {
        let isInTdoay = Locale.current.calendar.isDateInToday(dueDate)
        switch filter {
        case .today:
    ...
    private func saveReminder(_ reminder: Reminder, completion: (String?) -> Void) {
        ...
        readReminder(with: reminder.id) { ekReminder in
            ...
            ekReminder.alarms?.forEach { alarm in
                if let absoulteDate = alarm.absoluteDate {
